...
init_settings()
{
	return get_settings()->init(emu_path::g_nxbx_dir.string());
}
...
bool
settings::get_bool_value(const char *a_pSection, const char *a_pKey, bool a_nDefault)
{
	return m_ini.GetBoolValue(a_pSection, a_pKey, a_nDefault);
}
...
void
settings::set_bool_value(const char *a_pSection, const char *a_pKey, bool a_pValue)
{
	m_ini.SetBoolValue(a_pSection, a_pKey, a_pValue, nullptr, true);
}
