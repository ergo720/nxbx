...
void user::write(uint32_t addr, const uint32_t data)
{
	...
	uint32_t curr_chan_info = m_machine->get<pfifo>().regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_PUSH1)];
	...
	if (curr_chan_id == chan_id) {
		if (curr_chan_mode == (NV_PFIFO_CACHE1_PUSH1_MODE_MASK)) {
			...
			case NV_USER_DMA_PUT:
				...
				m_machine->get<pfifo>().regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_PUT)] = data;
...
uint32_t user::read(uint32_t addr)
{
	...
	if (curr_chan_id == chan_id) {
		if (curr_chan_mode == (NV_PFIFO_CACHE1_PUSH1_MODE_MASK)) {
			...
			{
			case NV_USER_DMA_PUT:
				value = m_machine->get<pfifo>().regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_PUT)];
				break;

			case NV_USER_DMA_GET:
				value = m_machine->get<pfifo>().regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_GET)];
				break;

			case NV_USER_REF:
				value = m_machine->get<pfifo>().regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_REF)];
				break;
...
auto user::get_io_func(bool log, bool is_be)
{
...
bool
user::update_io(bool is_update)
{
	bool log = module_enabled();
	bool is_be = m_machine->get<pmc>().endianness & NV_PMC_BOOT_1_ENDIAN24_BIG_MASK;
	if (!LC86_SUCCESS(mem_init_region_io(m_machine->get<cpu_t *>(), NV_USER_BASE, NV_USER_SIZE, false,
		{
