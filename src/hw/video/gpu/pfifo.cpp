...
void pfifo::write(uint32_t addr, const uint32_t data)
{
	...
	case NV_PFIFO_INTR_0:
		regs0[REGS0_PFIFO_idx(NV_PFIFO_INTR_0)] &= ~data;
	...
	case NV_PFIFO_CACHE1_DMA_PUSH:
		...
		regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_PUSH)] = (data & ~(NV_PFIFO_CACHE1_DMA_PUSH_STATE_MASK | NV_PFIFO_CACHE1_DMA_PUSH_BUFFER_MASK));
		break;

	case NV_PFIFO_CACHE1_DMA_PUT:
		regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_PUT)] = data;
		signal++;
		signal.notify_one();
		break;

	case NV_PFIFO_CACHE1_DMA_GET:
		regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_GET)] = data;
		signal++;
		signal.notify_one();
		break;

	default: {
		uint32_t reg_idx = ((addr - NV2A_REGISTER_BASE) & 0x1000) >> 12;
		uint32_t reg_offset = addr & 0xFFF;
		uint32_t low_offset = reg_idx == 0 ? REGS0_OFFSET_BASE : REGS1_OFFSET_BASE;
		uint32_t high_offset = reg_idx == 0 ? REGS0_OFFSET_END : REGS1_OFFSET_END;
		if (util::in_range(reg_offset, low_offset, high_offset - 4)) {
			regs[reg_idx][reg_offset >> 2] = data;
		}
		else {
...
uint32_t pfifo::read(uint32_t addr)
{
	...
	uint32_t high_offset = reg_idx == 0 ? REGS0_OFFSET_END : REGS1_OFFSET_END;
	if (util::in_range(reg_offset, low_offset, high_offset - 4)) {
		value = regs[reg_idx][reg_offset >> 2];
	}
	else {
...
{
	if ((
		((regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_PUSH0)] & NV_PFIFO_CACHE1_PUSH0_ACCESS_MASK) << 1) |
		(regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_PUSH)] & (NV_PFIFO_CACHE1_DMA_PUSH_ACCESS_MASK | NV_PFIFO_CACHE1_DMA_PUSH_STATUS_MASK))
		) ^
	...
	regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_PUSH)] |= NV_PFIFO_CACHE1_DMA_PUSH_STATE_MASK;

	uint32_t curr_pb_get = regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_GET)] & ~3;
	uint32_t curr_pb_put = regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_PUT)] & ~3;
	// Find the address of the new pb entries from the pb object
	dma_obj pb_obj = m_machine->get<nv2a>().get_dma_obj((regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_INSTANCE)] & NV_PFIFO_CACHE1_DMA_INSTANCE_ADDRESS_MASK) << 4);

	// Process all entries until the fifo is empty
	while (curr_pb_get != curr_pb_put) {
		if (curr_pb_get >= pb_obj.limit) {
			regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_STATE)] |= (NV_PFIFO_CACHE1_DMA_STATE_ERROR_PROTECTION << 29); // set mem fault error
		...
		uint32_t mthd_cnt = regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_STATE)] & NV_PFIFO_CACHE1_DMA_STATE_METHOD_COUNT_MASK; // parameter count of method
		if (mthd_cnt) {
			// A method is already being processed, so the following words must be its parameters

			regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_DATA_SHADOW)] = pb_entry; // save in shadow reg the current entry

			uint32_t cache1_put = regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_PUT)] & 0x1FC;
			uint32_t dma_state = regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_STATE)];
			...
			regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_METHOD(cache1_put >> 2))] = method_entry;
			regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DATA(cache1_put >> 2))] = pb_entry;
			...
			regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_STATE)] = dma_state;
			regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_DCOUNT)]++;
			...
			regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_RSVD_SHADOW)] = pb_entry; // save in shadow reg the current entry
			...
			if ((pb_entry & 0xE0000003) == 0x20000000) {
				...
				regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_GET_JMP_SHADOW)] = curr_pb_get;
				...
				if (regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_SUBROUTINE)] & 1) {
					regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_STATE)] |= (NV_PFIFO_CACHE1_DMA_STATE_ERROR_CALL << 29); // set call error
					err_handler("Pusher error: call command while another subroutine is already active");
					break;
				}
				regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_SUBROUTINE)] = curr_pb_get | 1;
				curr_pb_get = pb_entry & 0xFFFFFFFC;
			}
			else if (pb_entry == 0x00020000) {
				// return (nv1a+) -> restore pb get addr from subroutine return addr saved with a previous call
				// 00000000000000100000000000000000
				if ((regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_SUBROUTINE)] & 1) == 0) {
					regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_STATE)] |= (NV_PFIFO_CACHE1_DMA_STATE_ERROR_RETURN << 29); // set return error
					err_handler("Pusher error: return command while subroutine is not active");
					break;
				}
				curr_pb_get = regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_SUBROUTINE)] & ~3;
				regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_SUBROUTINE)] = 0;
			}
				...
				regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_STATE)] = mthd_state;
				regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_DCOUNT)] = 0;
			}
			else {
				regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_STATE)] |= (NV_PFIFO_CACHE1_DMA_STATE_ERROR_RESERVED_CMD << 29); // set invalid command error
	...
	regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_GET)] = curr_pb_get;
...
pfifo::worker(std::stop_token stok)
{
	...
	const auto lambda = [this](const char *msg) {
		regs1[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_DMA_PUSH)] |= NV_PFIFO_CACHE1_DMA_PUSH_STATUS_MASK; // suspend pusher
		// Currently disabled, because it's not thread safe yet
#if 0
		regs0[REGS0_PFIFO_idx(NV_PFIFO_INTR_0)] |= NV_PFIFO_INTR_0_DMA_PUSHER; // raise pusher interrupt
		m_machine->get<pmc>().update_irq();
#endif
		nxbx_fatal(msg);
		};
...
pfifo::reset()
{
	std::fill(std::begin(regs0), std::end(regs0), 0);
	std::fill(std::begin(regs1), std::end(regs1), 0);
	regs0[REGS1_PFIFO_idx(NV_PFIFO_CACHE1_STATUS)] = NV_PFIFO_CACHE1_STATUS_LOW_MARK_MASK;
	// Values dumped from a Retail 1.0 xbox
	regs0[REGS0_PFIFO_idx(NV_PFIFO_RAMHT)] = 0x00000100;
	regs0[REGS0_PFIFO_idx(NV_PFIFO_RAMFC)] = 0x008A0110;
	regs0[REGS0_PFIFO_idx(NV_PFIFO_RAMRO)] = 0x00000114;
}
