...
auto puser::get_io_func(bool log, bool is_be)
{
	if constexpr (is_write) {
		if (log) {
			return is_be ? nv2a_write<puser, uint32_t, &puser::write32<true>, big> : nv2a_write<puser, uint32_t, &puser::write32<true>, le>;
		}
		else {
			return is_be ? nv2a_write<puser, uint32_t, &puser::write32<false>, big> : nv2a_write<puser, uint32_t, &puser::write32<false>, le>;
		}
	}
	else {
		if (log) {
			return is_be ? nv2a_read<puser, uint32_t, &puser::read32<true>, big> : nv2a_read<puser, uint32_t, &puser::read32<true>, le>;
		}
		else {
			return is_be ? nv2a_read<puser, uint32_t, &puser::read32<false>, big> : nv2a_read<puser, uint32_t, &puser::read32<false>, le>;
		}
	}
}
